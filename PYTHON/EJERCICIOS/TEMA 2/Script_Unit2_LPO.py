...
if NonDNA:### if Non nucleotides are found, warns user and do not continue
  ...
  for Enz in REnz.keys(): #.keys to refer to the keys of the diccionaire and not the values
    if REnz[Enz] in Sequence:
      ...
      while not(End_of_Seq):
        print(Sequence.find(REnz[Enz],Position)+1,", ",end="")
